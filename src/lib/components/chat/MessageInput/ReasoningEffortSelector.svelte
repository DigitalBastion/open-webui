<script lang="ts">
	...
	$: efforts = intersectReasoningEffortLevels(
		selectedModelIds.map(
			(modelId) =>
				$models.find((model) => model.id === modelId)?.info?.meta?.capabilities?.reasoning_effort
		)
	);
...
{#if reasoningEffortSupported}
	<Dropdown bind:show align="end" side="top">
		<button
			type="button"
			class="flex max-w-[7.5rem] items-center gap-1 rounded-lg px-2 py-1 text-[13px] font-normal transition-colors duration-100 focus:outline-hidden {effectiveEffortSupported
		...
		<div slot="content">
			<DropdownMenu className="min-w-28">
				...
				{#if !effectiveEffortSupported}
					<p class="px-2 py-1 text-[11px] leading-tight text-amber-700 dark:text-amber-400" role="status">
